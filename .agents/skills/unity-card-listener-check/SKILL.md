...
- **Round-boundary flow events (GAME FLOW) accept the whole family** (`round_boundary_events`): 「回合开始」 → {`BeforeRoundFinished`, `OnRoundEnd`}; 「回合结束」 → `OnRoundEnd`; 「回合结束前」 → `BeforeStartCardReveal`; 「洗牌后」 → {`AfterShuffle`, `OnRoundEnd`}. The asset names lie — `BeforeRoundFinished.asset` is wired to `GameEventStorage.beforeRoundStart` and fires at combat start (GatherDecks, BEFORE the opening shuffle) AND at every round boundary right after `HandleNewRoundStart`'s per-round resets; `OnRoundEnd` fires after every start-card shuffle animation settles, BEFORE the resets (round-end effects read the completed round); `AfterShuffle` fires after the shuffle completes and the round's first card has reached the reveal zone. `BeforeStartCardReveal` (added 2026-09-05) is the true pre-shuffle round end: raised in `RevealNextCard` when the start card is the next reveal and BEFORE its shuffle; the reveal continues after the listener animations flush, so whatever a listener lands on the deck top reveals before the start card (FINAL_ESCORT's grave revive, RELIC_TALLY's tally).
- **Staging timepoint choice (② OnRoundEnd vs ③ beforeRoundStart)**: both are post-shuffle at boundary fires (resets don't reorder the deck), so both deliver next-round-first-reveal staging. Real differences: ③'s combat-start fire (GatherDecks, and there is NO shuffle before the first reveal — GatherDecks only merges) adds round-1 coverage, and ③ selects AFTER the per-round resets (base attack) while ② selects before them (includes this round's temp buffs). Use `BeforeStartCardReveal` to act on the closing round itself (FINAL_ESCORT, RELIC_TALLY).
