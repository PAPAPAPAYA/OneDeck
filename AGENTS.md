...
- **Shop top bar v3 (2026-09-19) + world scroll v4 (2026-09-21)**: §08 three-row column — row 0 = 离开商店 / reused combat avatar+username / reused combat HP pill / money / ❚❚ (one line); row 1 = rarity odds; row 2 = wins/hearts/income, rows 1-2 left-aligned. Glyphs: ♥/❚ baked into `NotoSansSymbols2 SDF`; 🜲 via the OFL `NotoSansSymbolsAlchemical SDF` last on the chrome font's fallback chain. 2026-09-21: the WHOLE bar is page content — `ShopChromeAnchor` deleted, root + `BandBottomWorldY` written once at Build, wheel scroll takes the bar away (scrolled-shop exit = scroll up or Space); a settled shop hides the canvas avatar/HP and shows the `ShopPageHud` world mirror (handoff rule `inCombat || inResult || (inShop && IsTransitioning)` + calibration details: `docs/ShopSystems.md`). 2026-09-23: top-bar/mirror placement + button feel live in `ShopLayoutConfigSO` (Resources/ShopLayoutConfig; OnValidate → `ShopChrome.ApplyLayout` / `ShopPageHud.ApplyMirrorLayout`), recipes shared in `ShopWorldWidgets`. Open: Shop panel header sits inside the chrome band (cosmetic; fix needs shop-pitch re-tune). Plans: `plans/plan-shop-topbar-combat-hud-reuse-2026-09-19.md`, `plans/plan-shop-topbar-world-scroll-2026-09-21.md`, `plans/plan-shop-layout-config-widget-factory-2026-09-23.md`.
