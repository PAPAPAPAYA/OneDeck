...
3. **防循环**: 同卡不放多个循环效果实例
4. **GameEvent.Raise 使用规则**: 仅当事件**不区分敌我**（如 `afterShuffle`、`onAnyCardRevealed`）时使用 `Raise()`；若事件带有 owner/opponent 语义（如 `onEnemyCurseCardRevealed`、`onMyPlayerTookDmg`），必须根据触发对象的实际阵营使用 `RaiseOwner()` 或 `RaiseOpponent()`，严禁直接调用 `Raise()`。
