...
- **Save the scene before any Test Runner run** (recurring — hit many times): with a dirty scene (the `*` in the Unity title bar) the test runner raises the "Scene(s) Have Been Modified — Save / Don't Save / Cancel" modal at start-up. That modal blocks the main thread, so the job ends as `Test job failed to initialize (tests did not start within timeout)` (or just hangs) **while `mcpforunity://editor/state` still reports `ready_for_tools: true`** — the state snapshot does not see the modal and `MainWindowTitle` stays unchanged, so it reads as "the runner is broken". Save the scene first (Ctrl+S, or keep it clean). After such a failure the dead job id remains as `tests.current_job_id` and blocks every later run — clear it with `run_tests` `clear_stuck: true`. Never auto-dismiss the dialog: `GameScene.unity` carries uncommitted user work and the only safe answer belongs to the user.
