...
- **Shop top bar v3 (2026-09-19) + world scroll v4 (2026-09-21)**: §08 three-row column — row 0 = 离开商店 / reused combat avatar+username / reused combat HP pill / money / ❚❚ (one line); row 1 = rarity odds; row 2 = wins/hearts/income, rows 1-2 left-aligned. Glyphs: ♥/❚ baked into `NotoSansSymbols2 SDF`; 🜲 via the OFL `NotoSansSymbolsAlchemical SDF` last on the chrome font's fallback chain. 2026-09-21: the WHOLE bar is page content — `ShopChromeAnchor` deleted, `ShopChrome.Build` positions the root once (`BandBottomWorldY` build-captured), and wheel scroll takes it away with the cards (scrolled-shop exit = scroll up or Space). In a settled shop the canvas avatar/HP hide and `ShopPageHud` (world mirror under the chrome root, `CanvasPxToWorld` calibration) shows instead; canvas pieces still glide as transition shared elements (handoff rule `inCombat || inResult || (inShop && IsTransitioning)`; HP pill parks at the shop anchor while hidden; world pill plays a ~0.3 s count-up on hpMax changes). Open: the Shop panel header still sits inside the chrome band (cosmetic; fixing it needs a shop-pitch re-tune). Plans: `plans/plan-shop-topbar-combat-hud-reuse-2026-09-19.md`, `plans/plan-shop-topbar-world-scroll-2026-09-21.md`.
