...
- **Extending**: new stat = one `CombatStatType` entry + one `CombatStatRegistry` entry + one `Record*()` call. Column order = `columnSortPriority`; row sort in `GetSessionRows()` (by `DamageDealtToOpponent` desc then faction). Registry flag `showPercentageOfTotal` renders the column as `"12 (34%)"` — the card's share of the combat-wide column total (set on both damage stats).
