...
- **Shop top bar v5 = authored prefab (2026-09-24)**: 7 chips + 2 buttons live in `Assets/Prefabs/ShopHud/ShopHudPage.prefab` (root `ShopHudPage` band contract 2.6/0.5/2 + `ShopHudBinder`; children = `Tpl_Chip`/`Tpl_WorldButton` variants; colors via `PaletteTint` slots, text via `HudTextBinding{key,format}` incl. TMP `<size>`, actions via `HudActionBinding`) — prefab-stage editing. `ShopChrome` is a loader (`Bootstrap(GameObject)`, frozen API): instantiate → place root + capture `BandBottomWorldY` once (page content). Avatar/HP mirror joined the prefab (09-24): `Tpl_Avatar`/`Tpl_HpPill` + `HudCountBinding`/`ShopMirrorScale`; `ShopPageHud`/`ShopLayoutConfigSO` deleted; canvas shop anchors invert the page children's world positions (`ShopTopBarLayout`). `ShopChrome.BandInsetFromTop` resolves page-first. `ShopWorldWidgets` + `chromeSprite/chromeFont` remain for `ShopSectionPanels`. Baseline probes diff empty. Open: Shop panel header sits inside the chrome band (cosmetic; fix needs shop-pitch re-tune). Plans: `plans/plan-shop-hud-prefab-widgets-2026-09-23.md` + `plans/plan-shop-mirror-prefab-2026-09-24.md` (supersede the 09-19/09-21/09-23 topbar plans).
- **Phase transition world-camera (2026-09-21)**: combat page one screen ABOVE the shop page (driver shifts `CombatUXManager`'s 7 world markers +pageH, removed when unavailable). `PhaseTransitionDriver` (runtime-built) wraps `PhaseManager` enter/exit: rig `DOMoveY`, avatar/HP pill glide, deck cards arc-fly + apex flip-down, enemy HUD slides down on landing; avatar+HP stay VISIBLE in Result. Trigger = 离开商店 `PhysButton`. Bypass: config off / batch / `overrideCombatSeed` / NullVisuals → legacy hard cut. Tuning: `PhaseTransitionConfigSO` (Resources). Docs: `docs/PhaseTransition.md`; plan `plans/plan-phase-transition-world-camera-2026-09-21.md`.
...
- **A dirty scene makes the Test Runner raise the "Scene(s) Have Been Modified" modal, which blocks the main thread** — the job ends as `Test job failed to initialize (tests did not start within timeout)` or just hangs while `ready_for_tools` still reports true. The dead job id then blocks every later run — clear it with `run_tests` `clear_stuck: true`. **The prompt cannot be intercepted** (verified in `com.unity.test-framework` source + by experiment, 2026-09-19: `SaveModifiedSceneTask` runs before any injectable callback, and preview scenes cannot be made active), so the ONLY reliable prevention is to save first: **before every `run_tests` call, run `EditorSceneManager.SaveOpenScenes()` (pre-approved, see Agent Behavior) as the immediately preceding step.** If the modal does appear it can only be dismissed by the user, so never click it. **Root cause FIXED 2026-09-19**: `HeadlessCombatRig` and `HeadlessCombatTestFixture` now build scratch objects in an `EditorSceneManager.NewPreviewScene()`, so a full run leaves GameScene clean (A/B measured 1 -> 0 dirt events, sim results byte-identical). Production code that creates scene objects inside a test (e.g. `ResultStatsPanel.Build`) or any real editor edit can still dirty it, so saving stays mandatory. Unity exposes no API to clear the dirty flag (only `MarkSceneDirty`).
- **Play-mode verification trap**: `Application.runInBackground` is `false`, so the player loop FREEZES while the Unity Editor window is unfocused (the normal case during MCP-driven testing). `Update()` stops running and any polled-phase component (`gamePhaseRef.Value()` vs a `_lastPhase` field) reads STALE — it looks exactly like a "component never repositions" bug and has already caused one phantom handoff anomaly. Always assert `Time.frameCount` is climbing in the same probe, and set `UnityEngine.Application.runInBackground = true` at the start of a verification session (runtime-only; does not touch ProjectSettings). Also: use the in-session Unity MCP tools (`execute_code` / `manage_editor` / `read_console` / `run_tests`) rather than hand-rolled HTTP helpers.
- **Stale-assembly trap (user ruling 2026-09-19: 改完文件一定要 refresh)**: a file-tool edit (`Edit`/`Write`/`sed`) to any `.cs` does **not** make Unity recompile — and this is true in Edit Mode, not just while in Play. The Test Runner then loads the PREVIOUS assembly: it reports the failure text of assertions you already replaced, and a "green" run can be green on old code (measured 2026-09-19: a `refresh_unity` landed 2 s after a run started, and the run reported a `StringAssert` message that had been rewritten to `Assert.AreEqual`). **Rule: after every `.cs` edit, `refresh_unity` (`compile: request`) before `run_tests`, and confirm `EditorApplication.isCompiling == false` AND the assembly DLL mtime > source file mtime** (`System.Type.GetType("X, Assembly-CSharp").Assembly.Location` vs the `.cs` path). `isCompiling == false` alone is not enough — it reads false while the stale assembly is still what the runner will use. Full-suite runs also need `run_tests` `init_timeout` raised (~180000; the 15 s default reports `Test job failed to initialize`).
- **Unfocused compile deferral (same trap, second form; user ruling 2026-09-19)**: while `InternalEditorUtility.isApplicationActive == false` (Unity is not the foreground window — the normal state during MCP-driven work) a compile request is only **QUEUED**, never executed; it compiles the instant the window regains focus. **The dangerous part**: `run_tests` whose filter matches **0 tests** returns `passed` WITHOUT forcing a recompile — a freshly written test class that has not been compiled yet reports `total: 0 … resultState: Passed`, a green that means nothing. **Rule: when a run of NEW tests looks suspiciously clean, check `isApplicationActive` and the assembly-vs-source mtime before believing it.** Clearing it: a `run_tests` over a NON-empty selection forces a synchronous recompile (this is how the deferral was cleared), otherwise wait for a click on the Unity window. The same deferral silently swallows **`EditorApplication.delayCall` work scheduled while unfocused** — the callback simply never fires (a scheduled batch scan ran while the window was active; an identical schedule minutes later, unfocused, produced no log line and no output file). Run long editor work synchronously from `execute_code` instead, or schedule it only when the window is active. Do NOT force focus with window automation — `SetForegroundWindow` is refused by Windows from a background process, and GUI automation is off-limits by rule. Measurements: `plans/plan-infinity-detection-2026-09-17.md` §23.3.
