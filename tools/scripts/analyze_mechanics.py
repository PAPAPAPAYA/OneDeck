...
class MechanicAnalyzer:
    ...
    def _analyze_effects(self, content, filename):
        ...
        effect_patterns = {
            ...
            r'ShieldAlterEffect': '护盾(Shield)',
            r'CardManipulationEffect': '卡牌操控',
            r'ChangeCardTarget': '改变目标(HeartChange)',
            r'ChangeHpAlterAmountEffect': '伤害/治疗量修�?,
            r'HPMaxAlterEffect': '最大生命值修�?,
            ...
            r'ConsumeStatusEffect': '消耗状�?,
            r'StatusEffectGiverEffect': '状态赋�?,
    ...
    def _analyze_costs(self, content, filename):
        ...
        cost_patterns = {
            ...
            r'CheckCost_Mana\((\d+)\)': '法力消�?,
            r'CheckCost_InGrave\(\)': '墓地条件',
            r'CheckCost_Revive\((\d+)\)': '复活消�?,
            r'CheckCost_Rested\((\d+)\)': '休息消�?,
        }
    ...
    def print_report(self):
        ...
        for eff, data in sorted(self.effect_types.items(), key=lambda x: -x[1]['count']):
            if data['count'] > 0:
                print(f'{eff}: {data["count"]} �?(涉及 {len(data["cards"])} 张卡�?')
        
        print('\n【三、目标与作用类型�?)
        print('-' * 60)
        for tgt, data in sorted(self.target_types.items(), key=lambda x: -x[1]['count']):
            print(f'{tgt}: {data["count"]} �?(涉及 {len(data["cards"])} 张卡�?')
        
        print('\n【四、费�?条件类型�?)
        print('-' * 60)
        for cost, data in sorted(self.cost_types.items(), key=lambda x: -x[1]['count']):
            print(f'{cost}: {data["count"]} �?(涉及 {len(data["cards"])} 张卡�?')
        
        print('\n【五、特殊机制�?)
        print('-' * 60)
        for mech, data in sorted(self.special_mechanics.items(), key=lambda x: -x[1]['count']):
            print(f'{mech}: {data["count"]} �?(涉及 {len(data["cards"])} 张卡�?')
        
        print('\n' + '=' * 60)
        print(f'总计分析卡牌: {len(self.card_categories)} 个类�? {sum(c["count"] for c in self.card_categories.values())} 张卡�?)
        print('=' * 60)
