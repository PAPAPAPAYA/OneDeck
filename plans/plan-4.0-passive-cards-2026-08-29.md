...
状态：**已实施（2026-08-29，用户明示「修改代码」后执行）**。开放问题裁决：#1 自定义顺序不含被动、强制垫底 ✓；#2 被动物理表现定案 2026-09-10 = 开场洗牌翻正、此后全程保持正面（洗牌不重置其已揭晓记忆）；#3 TAINT 相邻语义留待第 5 步前拍板。
...
2. **被动物理表现**（已定案 2026-09-10）：被动恒在墓地侧 = 视觉牌堆底部，**正面朝上并全程保持**。洗牌回调给被动独立分支（弧中点翻正、不重置已揭晓记忆 `everRevealed`），其余非强制覆盖路径被 never-cover 规则挡住，翻正动画每场战斗只播一次。实现：`CombatUXManager.PlayShuffleAnimationInternal` 的 `VISUAL-FIX(2026-09-10)` 块；同步 `docs/FaceDownFlipSystem.md`、`docs/RegressionChecklist.md` 行 88。
