...
2. **Interleaved playback (per-recorder)**: `PlayRecorderCoroutine` plays **all requests** on the current recorder sequentially, then recursively plays all unplayed children. This ensures all animations produced by the same effect instance are played contiguously, while reactive effects triggered by that instance are animated afterward.
