...
(Continue the full implementation:)
  ...
  - `public void RefreshCounter()` — `int total = ShopManager.me != null && ShopManager.me.deckSize != null ? ShopManager.me.deckSize.value : 0; int used = total - (ShopUXManager.Instance != null ? ShopUXManager.Instance.SpawnedEmptySlots.Count : 0); _deckCounter.text = FormatSlotCount(used, total);` (empty slots are spawned for every free deck slot, so `total - emptyCount` = occupied slots).
    - **Correction (2026-09-21):** the parenthetical assumption was wrong. Empty slots are one recessed frame per GRID slot — `SpawnEmptySlots` fills up to `deckSize` (ShopUXManager.InstantiatePlayerDeckPhysCards / SpawnAdditionalEmptySpaces) and clears only with the cards — so `total - emptyCount` was always 0 and the counter read `00/NN` for the whole run. Shipped form reads the deck SO: `int used = shop != null ? CountUsedSlots(shop.playerDeckRef, shop.DuplicateCopiesShareSlot) : 0;` backed by `UtilityFuncManagerScript.CountSlotOccupyingCards` — the same count the `ShopManager.BuyFunc` deck-full gate uses. See `docs/RegressionChecklist.md` row 109 and the `VISUAL-FIX(2026-09-21)` block in `ShopSectionPanels.RefreshCounter`.
