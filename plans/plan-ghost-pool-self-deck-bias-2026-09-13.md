# 对手 Ghost 缓存自牌偏置 (只匹配到 self-account 旧卡组)

日期: 2026-09-13
状态: **已实施 (2026-09-13)** — 方案 A+B + `fightOwnGhostsOnly` 只选自牌过滤; OpponentDeckCacheTests 13/13 绿, 全量 EditMode 514/515 (1 存量 Ignore)
现象: 新 run 匹配到的 ghost 卡组 username 全是 self-account (自己), 服务器上其他玩家 (player-C / player-B) 的卡组排不上队
...
**下载链路没有问题, 是选取偏置。** 本地缓存文件里 player-C 的卡组已经下载到位 (18 条), 但 `OpponentDeckCache.TakeCandidate` 按"列表第一条"取候选, 而旧的自牌 (self-account, 早期 includeSelf 时期合入) 全部排在列表前部, 每次 run 又从零开始消耗, 所以永远先命中自牌。
...
|----------|------|--------------|----------------|
| self-account | 17 | 0-5 | 最前 (deckId 2-19) |
| player-B | 2 | 0-1 | 中间 (deckId 20-21) |
| player-C | 18 | 0-6 | 最后 (deckId 22-47) |
...
5. **自牌只有两种情况轮得到**: 同一 run 内自牌按 session 耗尽 (一 run 每 session 一场, 实际到不了), 或 session 超出自牌覆盖范围 (self-account 缓存只到 session 5, session 6+ 才命中 #1299)。
