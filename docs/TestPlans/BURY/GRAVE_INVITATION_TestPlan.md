...
4. **Effect Execution**:
   ...
   - `BuryEffect.BuryTheirCards_BasedOnIntSO(OwnerInGraveAmountRef)` buries N enemy cards, where N equals the number of owner cards below the Start Card.
...
BuryTheirCards_BasedOnIntSO(OwnerInGraveAmountRef):
  N = OwnerInGraveAmountRef.value
...
> **Note:** `OwnerInGraveAmountRef` is refreshed by `ValueTrackerManager.UpdateAllTrackers()` inside `CostNEffectContainer.InvokeEffectEvent()` before the effect fires. The damage and bury are sequential calls within the **same** `effectEvent`, so they share the same tracker snapshot.
...
- **Zero Grave Case**: If `OwnerInGraveAmountRef = 0`, the bury call does nothing (amount = 0).
...
3. Construct a controlled `combinedDeckZone` with known Start Card position and owner/enemy card distribution.
...
|----|----------|-------------------|-----------------|------------------|
| A-1 | Base case: 2 owner cards in grave, 3 enemy eligible | combinedDeckZone = [OwnerCardA, OwnerCardB, Start, EnemyA, EnemyB, EnemyC]. Card in revealZone. | Opponent takes 4 damage. 2 enemy cards are buried to bottom. | Damage is fixed 4; bury count equals grave count. |
| A-2 | No owner cards in grave | Start Card at index 0 (bottom). All owner cards above it. | Opponent takes 4 damage. No enemy cards buried. | Zero grave count yields zero bury. |
| A-3 | More grave count than eligible enemy cards | OwnerInGraveAmountRef = 5, but only 2 eligible enemy cards exist. | Opponent takes 4 damage. Exactly 2 enemy cards buried. | Bury count clamps to eligible target count. |
| A-4 | Enemy cards include minions and bottom cards | 3 enemy cards: 1 minion, 1 at bottom, 1 eligible. OwnerInGraveAmountRef = 3. | Opponent takes 4 damage. Only 1 enemy card buried. | Minion and bottom filters work for enemy bury. |
...
| A-6 | Enemy perspective | Card belongs to enemy. Deck mirrors A-1. | Player (owner) takes 4 damage. 2 owner cards buried. | Faction perspective flips correctly for damage and bury. |
...
2. Ensure the test deck contains GRAVE_INVITATION and a mix of owner/enemy cards with known grave composition.
