...
- **Chrome v4 — whole bar is page content, wheel-scrolls away (2026-09-21, `plans/plan-shop-topbar-world-scroll-2026-09-21.md`)**: the entire top bar — chips, **离开商店**/**❚❚** buttons, and the avatar+username / HP pill as world mirrors (`ShopPageHud`, runtime-built under the `ShopChrome` root) — is fixed to the shop PAGE; the wheel scroll (camera rig Y) takes it away with the cards. The camera-pinned `ShopChromeAnchor` is deleted (`Build()` writes the root position once; `BandBottomWorldY` is build-captured). The canvas combat pieces hide in a settled shop (presenter handoff rule: `inCombat || inResult || (inShop && IsTransitioning)`) and fly as shared elements during transitions exactly as before. Scrolled-shop exit paths: scroll back up, or **Space** (the `phaseInfo` prompt already teaches the key) — a scrolled-down shop intentionally shows no exit button. HP pill value changes (HP utility buy/sell moves hpMax mid-visit) play a plain ~0.3 s integer count-up, no odometer strips.
...
- v0.14 · 2026-09-21 · Whole shop top bar becomes page content: `ShopChromeAnchor` deleted (`Build()` positions the chrome root once, `BandBottomWorldY` build-captured); new `ShopPageHud` mirrors the canvas avatar+username and HP pill as world objects under the chrome root (px→world calibration golden-tested in `ShopPageHudTests`); canvas pieces hide in a settled shop via the presenter handoff rule and still glide as transition shared elements; scrolled-shop exit = scroll up or Space. Plan: `plans/plan-shop-topbar-world-scroll-2026-09-21.md`. See 3.6.
